"""OpenRouter LLM provider for fact extraction.

This module implements the OpenRouter-based fact extraction provider using
...
class OpenRouterProvider(BaseLLMProvider):
    """LLM provider using OpenRouter multi-model aggregation via httpx."""
    ...
    def _map_fact_type(self, raw_type: str) -> FactType:
        """Maps OpenRouter extracted fact types to the internal FactType enum."""
        mapping = {
    ...
    async def extract_facts(self, payload: Dict[str, Any]) -> List[VerifiedBuildFact]:
        """Extracts structured facts from a raw webhook payload using OpenRouter.
...
"""
        try:
            ...
            if status_code != 200:
                logger.error("openrouter_provider.http_error", status_code=status_code, text=body_text)
                return []
                
            data = json.loads(body_text)
            content = data.get("choices", [{}])[0].get("message", {}).get("content")
            if not content:
                logger.warning("openrouter_provider.empty_response")
                return []
            ...
            try:
                ...
                if isinstance(parsed, dict):
                    ...
                    logger.error("openrouter_provider.unexpected_format", text=text)
                    return []
            except json.JSONDecodeError:
                logger.error("openrouter_provider.json_parse_error", text=text)
                return []
            ...
            for item in raw_facts:
                try:
                    ...
                    logger.warning("openrouter_provider.fact_validation_error", error=str(e), item=item)
                    continue
            
            logger.info("openrouter_provider.extraction_success", count=len(verified_facts))
            return verified_facts

        except Exception as e:
            logger.error("openrouter_provider.api_failure", error=str(e))
            return []

    async def get_available_models(self) -> List[str]:
        """Fetches live available models from OpenRouter API."""
        try:
            import asyncio
            ...
            if status_code == 200:
                ...
                logger.info("openrouter_provider.models_fetched", count=len(models))
                return models
            logger.warning("openrouter_provider.models_fetch_failed", status_code=status_code, text=body_text)
            return []
        except Exception as e:
            logger.error("openrouter_provider.models_fetch_error", error=str(e))
            return []
